...
if (command === "list-users") {
  console.log("\n=== Current Users ===");
  const users = db
    .prepare("SELECT id, email, first_name, last_name, ssn FROM users")
    .all();
  if (users.length === 0) {
  ...
  if (sessions.length === 0) {
    ...
    sessions.forEach((session) => {
      ...
      console.log(
        `User: ${session.email}, Expires: ${session.expires_at} ${
  ...
  if (!email) {
    ...
    if (user) {
      ...
      db.exec(
        `DELETE FROM transactions WHERE account_id IN (SELECT id FROM accounts WHERE user_id = ${user.id})`,
      );
